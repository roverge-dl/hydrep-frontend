...
const Documents: React.FC<StepChildProps> = ({ setFormData }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const existingDocs = data.data.requirements
          .filter((r: any) => r.status === "uploaded" && r.document)
          .reduce(
            (acc: any, r: any) => ({
              ...acc,
  ...
  const handleUpload = async (
    ...
    try {
      // Call API
      const response = await uploadRequirementDocument(reqId, payload.file, {
...
const RequirementItem = ({
  ...
  const [localData, setLocalData] = useState<DocPayload>({
    ...
    dateIssued: requirement.document?.date_issued
      ? requirement.document.date_issued.split("T")[0]
      : "",
    expiryDate: requirement.document?.expiry_date
      ? requirement.document.expiry_date.split("T")[0]
      : "",
  });
    ...
    <div
      ...
      <div
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="font-semibold text-hdark-500 text-sm">
                {requirement.title}
              </h4>
        ...
        <div className="flex items-center gap-3">
          {hasDoc && (
            <span className="text-xs font-medium text-green-600 hidden sm:block">
              Completed
            </span>
          )}
          {isOpen ? (
            <BiChevronUp className="text-gray-400" />
          ) : (
