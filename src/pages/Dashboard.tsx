            ...
            {stats?.recentApplications?.length > 0 ? (
              <div className="space-y-3">
                {stats.recentApplications.map((app: any) => (
                  <div
                    key={app.id}
                    className="p-3 border border-hdark-300 rounded-lg flex justify-between items-center">
                    <div>
                    ...
                    <span
                      className={`px-2 py-1 capitalize rounded text-xs ${app.status === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
                      {app.status}
                    </span>
                  </div>
                ))}
